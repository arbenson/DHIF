#pragma once
#ifndef DHIF_VEC3T_HPP
#define DHIF_VEC3T_HPP 1
...
public:
    ...
    { _v[0] =c._v[0]; _v[1] =c._v[1]; _v[2] =c._v[2]; return *this; }
    Vec3T& operator+=( const Vec3T& c )
    { _v[0]+=c._v[0]; _v[1]+=c._v[1]; _v[2]+=c._v[2]; return *this; }
    Vec3T& operator-=( const Vec3T& c )
    { _v[0]-=c._v[0]; _v[1]-=c._v[1]; _v[2]-=c._v[2]; return *this; }
    ...
    //-----------LENGTH...
    F l1( void ) const
    { F sum=F(0); for(int i=0; i<3; i++) sum=sum+abs(_v[i]); return sum; }
    F linfty( void ) const
    {
        F cur=F(0);
        for(int i=0; i<3; i++)
            cur=std::max(cur,abs(_v[i]));
        return cur;
    }
    F l2( void ) const
    {
...
//-----------LEX COMPARE
template <class F> inline bool operator==
(const Vec3T<F>& a, const Vec3T<F>& b) {
...
template <class F> inline Vec3T<F> operator+
(const Vec3T<F>& a, const Vec3T<F>& b) {
...
template <class F> inline Vec3T<F> operator^
(const Vec3T<F>& a, const Vec3T<F>& b) {
    return Vec3T<F>(a(1)*b(2)-a(2)*b(1), a(2)*b(0)-a(0)*b(2),
                    a(0)*b(1)-a(1)*b(0));
}

template <class F> inline Vec3T<F> cross
(const Vec3T<F>& a, const Vec3T<F>& b) {
    return a^b;
}
...
template <class F> inline Vec3T<F> vec3max(const Vec3T<F>& a, F scl) {
    return Vec3T<F>(std::max(a[0], scl), std::max(a[1], scl),
                    std::max(a[2], scl));
}

template <class F> inline Vec3T<F> vec3min(const Vec3T<F>& a, F scl) {
    return Vec3T<F>(std::min(a[0], scl), std::min(a[1], scl),
                    std::min(a[2], scl));
}


//-------------ew NUMERICAL OPS
template <class F> inline Vec3T<F> ewmin
(const Vec3T<F>& a, const Vec3T<F>& b) {
    return Vec3T<F>(std::min(a[0], b[0]), std::min(a[1], b[1]),
                    std::min(a[2], b[2]));
}

template <class F> inline Vec3T<F> ewmax
(const Vec3T<F>& a, const Vec3T<F>& b) {
    return Vec3T<F>(std::max(a[0], b[0]), std::max(a[1], b[1]),
                    std::max(a[2], b[2]));
}
...
template <class F> inline Vec3T<F> ewmul
(const Vec3T<F>&a, const Vec3T<F>& b) {
    return Vec3T<F>(a[0] * b[0], a[1] * b[1], a[2] * b[2]);
}

template <class F> inline Vec3T<F> ewdiv
(const Vec3T<F>&a, const Vec3T<F>& b) {
    return Vec3T<F>(a[0] / b[0], a[1] / b[1], a[2] / b[2]);
}
...
//---------------INOUT
template <class F> std::istream& operator>>
(std::istream& is, Vec3T<F>& a) {
    for(int i=0; i<3; i++)
        is >> a[i];
    return is;
}

template <class F> std::ostream& operator<<
(std::ostream& os, const Vec3T<F>& a) {
    for(int i=0; i<3; i++)
        os << a[i] << " ";
    return os;
}
