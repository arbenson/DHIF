...
Real Householder( const int m, Real* buffer )
{
    ...
    Real alpha = buffer[0];
    Real norm = DHIF::blas::Nrm2( m-1, &buffer[1], 1 );

    Real beta;
    if( alpha <= 0 )
        beta = DHIF::lapack::SafeNorm( alpha, norm );
    else
        beta = -DHIF::lapack::SafeNorm( alpha, norm );

    // Avoid overflow by scaling the vector
    const Real safeMin = DHIF::lapack::MachineSafeMin<Real>() /
                         DHIF::lapack::MachineEpsilon<Real>();
    int count = 0;
    if( DHIF::Abs(beta) < safeMin )
    {
        Real invOfSafeMin = static_cast<Real>(1) / safeMin;
        do
        {
            ++count;
            DHIF::blas::Scal( m-1, invOfSafeMin, &buffer[1], 1 );
            alpha *= invOfSafeMin;
            beta *= invOfSafeMin;
        } while( DHIF::Abs( beta ) < safeMin );

        norm = DHIF::blas::Nrm2( m-1, &buffer[1], 1 );
        if( alpha <= 0 )
            beta = DHIF::lapack::SafeNorm( alpha, norm );
        else
            beta = -DHIF::lapack::SafeNorm( alpha, norm );
    ...
    DHIF::blas::Scal( m-1, static_cast<Real>(1)/(alpha-beta), &buffer[1], 1 );
...
std::complex<Real> Householder( const int m, std::complex<Real>* buffer )
{
    ...
    Real beta;
    if( real(alpha) <= 0 )
        beta = DHIF::lapack::SafeNorm( real(alpha), imag(alpha), norm );
    else
        beta = -DHIF::lapack::SafeNorm( real(alpha), imag(alpha), norm );
    ...
    if( DHIF::Abs(beta) < safeMin )
    {
        ...
        {
            ++count;
            DHIF::blas::Scal( m-1, Scalar(invOfSafeMin), &buffer[1], 1 );
            alpha *= invOfSafeMin;
            beta *= invOfSafeMin;
        } while( DHIF::Abs( beta ) < safeMin );
    ...
    DHIF::blas::Scal( m-1, static_cast<Scalar>(1)/(alpha-beta), &buffer[1], 1 );
...
{
#ifndef RELEASE
    CallStackEntry entry("mat_tools::ApplyPackedQFromLeft");
    if( B.Type() != GENERAL )
...
{
#ifndef RELEASE
    CallStackEntry entry("mat_tools::ApplyPackedQAdjointFromLeft");
    if( B.Type() != GENERAL )
        throw std::logic_error("B must be a full dense matrix");
    if( B.Height() != s+t )
        throw std::logic_error("B is not the correct height");
...
{
#ifndef RELEASE
    CallStackEntry entry("mat_tools::ApplyPackedQFromRight");
    if( B.Type() != GENERAL )
        throw std::logic_error("B must be a full dense matrix");
    if( B.Width() != s+t )
...
{
#ifndef RELEASE
    CallStackEntry entry("mat_tools::ApplyPackedQAdjointFromRight");
    if( B.Type() != GENERAL )
        throw std::logic_error("B must be a full dense matrix");
    if( B.Width() != s+t )
        throw std::logic_error("B is not the correct width");
...
{
#ifndef RELEASE
    CallStackEntry entry("mat_tools::PrintPacked");
#endif
    os << msg << "\n";
